...
local BeginAutoroutine, InterruptAutoroutine, YieldAutoroutine, GetRunningAutoroutine, RegisterReaction, RegisterMemberFunctionReaction, tPool = 
	DGV.BeginAutoroutine, DGV.InterruptAutoroutine, DGV.YieldAutoroutine, DGV.GetRunningAutoroutine, DGV.RegisterReaction, DGV.RegisterMemberFunctionReaction, DGV.tPool
...
function DugisArrow:Initialize()
	...
	local function PlaceWaypoint(point)
		...
		if guideIndex and #DGV:getCoords(guideIndex) == 1 and not DugisGuideUser.FinalizeWaypoint then virtual = 1 end --need this for single waypoints with routes. 
		...
		if UseCarboniteArrow() then
			if Nx.Travel.GOpts["MapRouteUse"] then Nx.Travel.GOpts["MapRouteUse"] = false end
			
			local opts = {}
			local map = Nx.Map:GetMap (1)
			opts.title = desc
			point.carbonite = map:SetTargetXY (Nx.AIdToId[mapID], x, y, opts.title, true)
	...
	local function GetPOIWaypointDescription(poiButton)
		...
		elseif poiButton and poiButton.type == QUEST_POI_COMPLETE_SWAP then
			desc = "|cfff0c502"..GetQuestLogCompletionText(logIndex).."|r"
		elseif poiButton then
			desc = "|cfff0c502"..GetQuestLogTitle(logIndex).."|r"
		end
		return desc
	end
	...
	local function SetWaypointInfo(mapID, mapFloor, x, y, desc, guideIndex, isWTag, questId, forceCalculation)
		...
		if DugisArrow:getNumWaypoints()>0 and not forceCalculation then
	...
	function DugisArrow:WaypointsChanged()
		...
		if guideIndex then
			...
			if dist and dist > 100 and not DugisGuideViewer:IsPlayerAtBlizzardDestination() then
...
DGV:DebugFormat("DiffRecalculationPoints", "diffDetected", diffDetected)
		...
		local function RecalculateRoutes()
			...
			if recalculationQueue then 
				BeginAutoroutine("RecalculateRoutes", RecalculateRoutesRoutine):OnCompletion(DiffRecalculationPoints)
			end
		end
