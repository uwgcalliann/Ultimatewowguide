...
function MV:Initialize()
	...
	function MV:CreateFrame()
		...
		if DugisGuideViewer_ModelViewer then 
			...
			MV.Frame.model:SetSize(149, 190)
			MV.Frame.model:SetPoint("CENTER", 2, -5)	
			...
			MV.Frame.model:SetScript("OnMouseDown", function() MV.Frame.StartMoving(MV.Frame) end)
			MV.Frame.model:SetScript("OnMouseUp", function() MV.Frame.StopMovingOrSizing(MV.Frame) end)
			...
			MV.Frame.close:SetScript("OnClick", function() MV:HideModel()  end)
	...
	function MV:Load()
		...
		function MV:RegisterObjects(objFunc)
			...
			lineEnd = 0
			lineStart = 0
			while (lineStart and safetyCount < 20000) do
		...
		function MV:RegisterNPCs(npcFunc)
			...
			lineEnd = 0
			lineStart = 0
			while (lineStart and safetyCount < 40000) do
	...
	function MV:Unload()
		if DGV:UserSetting(DGV_UNLOADMODULES) or not DGV:UserSetting(DGV_ENABLENPCNAMEDB) then
			MV.objDB = nil
			MV.npcDB = nil
		end
		MV.data.models = nil
