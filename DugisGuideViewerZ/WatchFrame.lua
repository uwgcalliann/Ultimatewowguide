...
local function OnDragStart(frame)
	if not WF:ShouldModWatchFrame() or not DGV:UserSetting(DGV_MOVEWATCHFRAME) then return end
	frame:StartMoving();
	frame.isMoving = true;
...
function WF:Initialize()
	...
	function DGV:WatchFrameOnDragStart()
		OnDragStart(WatchFrame)
	end
	
	function DGV:WatchFrameOnDragStop()
		OnDragStop(WatchFrame)
	end
	...
	local function UnsnapWatchFrame()
		if WF:ShouldModWatchFrame() then
			--if DGV:UserSetting(DGV_WATCHFRAMEBORDER) or not DGV:UserSetting(DGV_DISABLEWATCHFRAMEMOD) then 
			...
			SetMovable()
			if not DGV:UserSetting(DGV_MOVEWATCHFRAME) then
	...
	local function SnapWatchFrame(forceLoaded)
		if WF:ShouldModWatchFrame(forceLoaded) then
			...
			if DugisGuideUser.DefaultWatchFrameHeight and DugisGuideUser.DefaultWatchFrameHeight < (WatchFrame:GetTop() - WatchFrame:GetBottom()) then			
				DugisGuideUser.DefaultWatchFrameHeight = WatchFrame:GetTop() - WatchFrame:GetBottom()
			elseif not DugisGuideUser.DefaultWatchFrameHeight then 
				DugisGuideUser.DefaultWatchFrameHeight = WatchFrame:GetTop() - WatchFrame:GetBottom()
			end			
		end
	end
	...
	function WF:OnWatchFrame_Update()
		...
		then
			...
			DugisGuideViewer.BACKGRND_PATH, DugisGuideViewer:GetBorderPath(), 10, 4, 12, 6)
		...
		if WF:ShouldModWatchFrame() then
			...
			-- DGV:DebugFormat("WF:OnWatchFrame_Update 2", "WatchFrame:GetTop()", WatchFrame:GetTop())
			...
			if nextAnchor and anchorBottom then
				...
				if DugisGuideUser.DefaultWatchFrameHeight then 
					newWatchFrameHeight = math.min(DugisGuideUser.DefaultWatchFrameHeight, newWatchFrameHeight)
				end
		...
		if DGV:GetDB(DGV_WAYPOINTSON) and DGV.chardb.EssentialsMode == 1 then
			DGV.Modules.QuestPOI:ObjectivesChangedDelay(3)
		end
	end
