...
local function SetBindings(buttonName, ...)
	for i=1,select("#", ...) do
		SetBindingClick((select(i, ...)), buttonName)
	end
end
...
local function OnEvent(self, event)
	if event == "PLAYER_REGEN_ENABLED" then
		...
		if texture and DGV:UserSetting(DGV_ITEMBUTTONON) then
			...
			actionFrame:Show()
			texture = nil
			SetBindings("DugisGuideViewerActionItemFrame" , GetBindingKey("DUGI_ITEM_BUTTON"))
		else
...
local function SetQuestItemFrame(logIndex, icon, charges)
	if DGV:UserSetting(DGV_ITEMBUTTONON) then
		if InCombatLockdown() then 
			...
			actionFrame:Hide()
			SetBindings("DugisGuideViewerQuestItemFrame" , GetBindingKey("DUGI_ITEM_BUTTON"))
		end
	end 
end
