...
function Taxi:Initialize()
	...
	local function BacktrackCharacterPath(contData, m1, f1, x1, y1, mTrans, m2, f2, x2, y2, ...)
		...
		if not contData[mTrans] then return end
			...
			return mTrans,resultF,resultX,resultY,tPool(recursiveResult) 
		--end
	end
...
-- 		end
	...
	local loadConstant = 10
	local penaltyConstant = 30
		...
		while true do
			...
			if chunk then
				...
				local telBuild = RouteBuilders.UnboundTeleport:Build(
					...
					m1, f1, x1, y1, m2, f2, x2, y2, key, data)
				if telBuild then
	...
	function RouteBuilders.InstancePortals:IsBest(route)
		if route.currentBest.builder==RouteBuilders.InstancePortals then
