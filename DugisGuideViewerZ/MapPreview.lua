...
function MP:Initialize()
	...
	local function IterateNonPreviewElements(delegate, unconditional)
		...
		delegate(WorldMapShowDropDown, 7)
		--delegate(WorldMapShowDigSites, 8)
		delegate(WorldMapFrameAreaLabel, 8)
		delegate(WorldMapFrameAreaDescription, 9)
		if DGV.CoordsFrame then
			delegate(DGV.CoordsFrame, 10)
		end
		delegate(WorldMapTrackQuest, 11)
		...
		if unconditional or (GetCurrentMapZone() > 0 and DGV:MapHasOverlays()) then
			for i=1, numDetails do
				delegate(_G["WorldMapDetailTile"..i], 11+i)
			end
		end
		local levels = GetNumDungeonMapLevels()
		if levels and levels > 0 then
			delegate(WorldMapLevelDropDown, 11+numDetails+1)
		end
	end
	...
	local function IsWaypointWithinCurrentFloorArea()
		local wp = DGV.DugisArrow.waypoints[1]
		...
		if not MP.FadeInAnimationGroup then
			...
			fadeInAnimation:SetScript("OnPlay", function()
				...
				if DGV.DugisArrow.waypoints and #DGV.DugisArrow.waypoints>0 then
					if not IsWaypointWithinCurrentFloorArea() then
						local m,f = DGV.DugisArrow.waypoints[1].map, DGV.DugisArrow.waypoints[1].floor or 0
						SetMapByID(m)
			...
			MP.WaitAnimation:SetScript("OnPlay", function(self)
				...
				if DGV.DugisArrow.waypoints and #DGV.DugisArrow.waypoints>0 then
					...
					local wpx, wpy, wpm, wpf = DGV.DugisArrow.waypoints[1].x/100, DGV.DugisArrow.waypoints[1].y/100, DGV.DugisArrow.waypoints[1].map, DGV.DugisArrow.waypoints[1].floor
					...
					if not IsWaypointWithinCurrentFloorArea() then
						...
						SetDungeonMapLevel(DGV.DugisArrow.waypoints[1].floor or 0)
					end
				end
